...
/*   Updated: 2025/03/18 06:29:34 by abchikhi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	prepare_camera(t_render_range *rr,
		t_vec3 up, t_vec3 right, t_vec3 forward)
{
	t_vec3	horizontal;
	t_vec3	vertical;
	...
	viewport_height = 2.0;
	viewport_width = viewport_height * ((double)sys_data()->screen_width
			/ (double)sys_data()->screen_height);
	horizontal = mult_(right, viewport_width);
	vertical = mult_(up, viewport_height);
	rr->h_pixel_delta = div_(horizontal, sys_data()->screen_width);
	rr->v_pixel_delta = div_(vertical, sys_data()->screen_height);
